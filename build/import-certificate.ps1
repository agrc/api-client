...
# --- Script Body ---
Write-Host "Starting code signing certificate import and linking..."
...
try {
    ...
    $kmsKeyPath = $env:GCP_KEY_PATH
    if ([string]::IsNullOrWhiteSpace($kmsKeyPath)) {
        Write-Warning "GCP_KEY_PATH environment variable is not set; skipping repairstore."
    }
    else {
